...
function App() {
  ...
  async function handleDeleteCustomer(customerId, customerName) {
    ...
    if (isConfirmed) {
      ...
      if (error) { alert('Lỗi! Không thể xóa khách hàng.'); } else { fetchDashboardData(); }
    }
  }
  ...
  const AlertBadge = ({ days_overdue }) => { if (!days_overdue || days_overdue <= 0) { return <span className="alert-badge green">Đúng hạn</span>; } const level = Math.floor((days_overdue - 1) / 30) + 1; return <span className="alert-badge red" title={`Quá hạn ${days_overdue} ngày`}>{level}</span>; };
    ...
    return (
      <div className="container">
        ...
        <div className="summary-section">
          <div className="summary-box"><h4>Tổng Tiền Đã Vay</h4><p>{summaryData.total_loaned.toLocaleString()} VND</p></div>
          <div className="summary-box"><h4>Tổng Lãi Tạm Tính Đến Nay</h4><p>{Math.round(summaryData.total_accrued_interest).toLocaleString()} VND</p></div>
        </div>
        ...
        <div className="table-container">
          <table>
            <thead><tr><th>Tên khách hàng</th><th>Tình trạng</th><th>Số điện thoại</th><th>Hành động</th></tr></thead>
            <tbody>
